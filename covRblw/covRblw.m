%  [C,shrink] = covRblw(X,varargin)
% 
%  DESCRIPTION
%  Computes the covariance matrix from a matrix of raw scores X using the 
%  Rao-BlackWell Ledoit-Wolf (RBLW) method described in Chen et al. (2010).
%
...
%  introducing bias. 
%
%  COVRBLW returns the shrunk covariance matrix C with minimised MSE and
...
%
%  FUNCTION CALL
%  1. [C,shrink] = covRblw(X)
%  2. [C,shrink] = covRblw(...,PROPERTY,VALUE)
%       ¬ 'ShrinkCoef'
%
%  REFERENCES
...
%  - Changed function call to set the shrinkage coefficient as input property
%    rather than input argument.
%
%  VERSION 1.0
%  Date: 28 Oct 2021
%  Author: Guillermo Jimenez Arranz
...
function [C,shrink] = covRblw(X,varargin)
...
if isempty(shrink)  
    ...
    % Shrinkage Coefficient
    shrink = (a*tr_s2 + trs_2 )/(b*(tr_s2 - trs_2/N));
end
